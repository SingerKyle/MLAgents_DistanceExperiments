...
public class AgentControls : Agent
{
    ...
    //Enemy Agemt Controller
    public EnemyController EnemyController;
    
    // Target transform and sensor component
    [SerializeField] private Transform Target;
    [SerializeField] private RayPerceptionSensorComponent3D sensor;

    // Value associated with Minkowski
    [SerializeField] private int MinkowskiValue = 4;

    // Reward count to limit reward from looking at object
    private int RewardCount = 0;
    
    // Distance type for Enum
    ...
    public override void OnEpisodeBegin()
    {
        ...
        for (int i = 0; i < Obstacles.Count; i++)
        {
            if (counter <= 10)
            {
                if (!constant.CheckOverlaps(AgentPosition, Obstacles[i].transform.position, 2f))
                {
        ...
        for (int i = 0; i < Obstacles.Count; i++)
        {
            if (counter <= 10)
            {
                if (!constant.CheckOverlaps(TargetPosition, Obstacles[i].transform.position, 3f))
                {
                    TargetPosition = new Vector3(Random.Range(-9, 9), Random.Range(0, 11), Random.Range(-9, 9));
                    i--;
                }

                if (!constant.CheckOverlaps(TargetPosition, AgentPosition, 1f))
                {
